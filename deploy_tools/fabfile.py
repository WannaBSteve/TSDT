...
def deploy():
    # site_foler = f'/home/{env.user}/sites/{env.host}'
    site_foler = f'/root/sites/{env.host}'
    source_folder = site_foler + '/source'
    _create_directory_structure_if_necessary(site_foler)
    _get_latest_source(source_folder)
    _update_settings(source_folder, env.host)
    # _update_virtualenv(source_folder)
    _update_conda_env(source_folder)
    _update_static_files(source_folder)
    _update_database(source_folder)

def _create_directory_structure_if_necessary(site_foler):
    for subfolder in ('database', 'static', 'virtualenv', 'source'):
        run(f'mkdir -p {site_foler}/{subfolder}')
...
def _update_static_files(source_folder):
    run(f'cd {source_folder} && /root/miniconda3/envs/{CONDA_ENV_NAME}/bin/python manage.py collectstatic --noinput')

# def _update_database(source_folder):
#     run(f'cd {source_folder} && ../virtualenv/bin/python manage.py migrate --noinput')

def _update_database(source_folder):
    run(f'cd {source_folder} && /root/miniconda3/envs/{CONDA_ENV_NAME}/bin/python manage.py migrate --noinput')
